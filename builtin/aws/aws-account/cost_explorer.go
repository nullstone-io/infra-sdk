...
// CostExplorerCapabilities describes the Cost Explorer tier: at most 2 group definitions per
// request, billed and effective cost only, no charge category or resource grouping.
func CostExplorerCapabilities() infra_sdk.CostCapabilities {
	return infra_sdk.CostCapabilities{MaxGroupBy: 2, Metrics: SupportedMetrics()}
}

// CostExplorerCoster answers cost queries from the Cost Explorer API. It is the baseline source
// of Coster; use Coster unless Cost Explorer is wanted explicitly.
type CostExplorerCoster struct {
	Accessor infra_sdk.AwsAccessor
}

func (c CostExplorerCoster) ProviderType() string { return "aws" }

func (c CostExplorerCoster) GetCosts(ctx context.Context, query infra_sdk.CostQuery) (*infra_sdk.CostResult, error) {
...
func filterTagToExpression(filterTag infra_sdk.CostFilterTag) *cetypes.Expression {
	key := ptr(aws_names.UniversalTag(filterTag.Key).ToAws())
	...
	for _, cur := range groupBy {
		if cur.Dimension != "" {
			defs = append(defs, cetypes.GroupDefinition{
				Key:  ptr(aws_names.UniversalDimension(cur.Dimension).ToAws()),
				Type: cetypes.GroupDefinitionTypeDimension,
			})
		} else if cur.TagKey != "" {
			defs = append(defs, cetypes.GroupDefinition{
				Key:  ptr(aws_names.UniversalTag(cur.TagKey).ToAws()),
